  ...
  constructor(string: string) {
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if (parsed[i][0] === "M") {
        ...
        this.functions.push(null);
        if (i === 0) {
          this.initial_point = { x: cur[0], y: cur[1] };
        }
        //lineTo
        ...
        this.functions.push(
          new LinearPosition(cur[0], ringStart[0], cur[1], ringStart[1], true)
        );
  ...
  public getParts = () => {
    ...
    for (var i = 0; i < this.functions.length; i++) {
      if (this.functions[i] !== null) {
        const f = this.functions[i] as Properties;
        const properties: PartProperties = {
          start: f.getPointAtLength(0),
          end: f.getPointAtLength(
            this.partial_lengths[i] - this.partial_lengths[i - 1]
          ),
          length: this.partial_lengths[i] - this.partial_lengths[i - 1],
          getPointAtLength: f.getPointAtLength,
          getTangentAtLength: f.getTangentAtLength,
          getPropertiesAtLength: f.getPropertiesAtLength,
          closed: !!(f as any).close,
        };
